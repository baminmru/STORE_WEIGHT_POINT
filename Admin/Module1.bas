...
Public Const ARMID As String = "{705AD25F-6514-4B4E-8E00-9CD0DC418A6D}"
...
again:
 ...
 Dim rs As ADODB.Recordset
  ...
  If GetSetting("MTZ", "CONFIG", "IMAGEPATH", "") = "" Then
    Call SaveSetting("MTZ", "CONFIG", "IMAGEPATH", App.path & "\IMAGES\")
  End If
  
  If GetSetting("MTZ", "CONFIG", "LAYOUTS", "") = "" Then
      Call SaveSetting("MTZ", "CONFIG", "LAYOUTS", App.path & "\LAYOUTS\")
  End If
  
  If GetSetting("MTZ", "CONFIG", "TEMPPATH", "") = "" Then
    Call SaveSetting("MTZ", "CONFIG", "TEMPPATH", App.path & "\TMP\")
  End If
  
  
  Dim tmpdir As String, tmpfile As String
  
  tmpdir = GetSetting("MTZ", "CONFIG", "TEMPPATH", App.path & "\TMP\")
  tmpfile = Dir(tmpdir & "*.*", vbNormal)
  
  
  frmSplash.lblWarning = "Удаление временных файлов"
  DoEvents
...
closefile:
  ...
  frmSplash.lblWarning = "Иниициализация меню"
  DoEvents
  Dim i As Long
