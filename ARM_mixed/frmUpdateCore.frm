...
Attribute VB_HelpID = 725

Option Explicit
'обновление данных по заказу на приемку
    ...
    If Not qry Is Nothing Then
      ...
      "Обновить данные по текущем состоянию документа ?", vbYesNo + vbExclamation, "ВНИМАНИЕ") = vbYes Then
        If chkInsert.Value = vbChecked Then
          OK = CleanRCVAtCore(qry)
        End If
        
        If Not OK Then
          MsgBox "Ошибка при удалении данных"
        End If
        ...
        For i = 1 To qry.ITTIN_QLINE.Count
          For j = 1 To qry.ITTIN_QLINE.Item(i).ITTIN_PALET.Count
            ...
            If chkInsert.Value = vbChecked Then
              ...
              OK = False
              While Not OK
                OK = SaveRCVRowToCore(qry, qry.ITTIN_QLINE.Item(i), qry.ITTIN_QLINE.Item(i).ITTIN_PALET.Item(j), pal.BufferZonePlace, txtQryCode.Text)
                If Not OK Then
                   MsgBox "Ошибка при сохранении данных в CORE. " & pal.TheNumber.brief & vbCrLf & "Будет проведена повторная попытка записи данных.", vbOKOnly
                   Dim conn As Object
                   Set conn = GetCoreConn(True)
                End If
              Wend
            Else
              OK = UpdateMyPalet(qry.ITTIN_QLINE.Item(i).ITTIN_PALET.Item(j))
                If Not OK Then
                   MsgBox "Ошибка при сохранении данных в CORE. " & pal.TheNumber.brief & vbCrLf & "Будет проведена повторная попытка записи данных.", vbOKOnly
                   Dim conn5 As Object
                   Set conn5 = GetCoreConn(True)
                End If
            End If
          Next
        Next
