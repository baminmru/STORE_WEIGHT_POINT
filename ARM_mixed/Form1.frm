...
Attribute VB_HelpID = 730
'Форма для процесса взвешивания палддонов
...
' сохранение информации о поддоне в базе данных
...
' "{588C5203-1E59-408E-92A1-B3DFED8C19FA}" 'Списана
      ...
      ' проверяем что поддон пустой
      If poddon.Application.StatusID <> "{93E3DE6D-AB8D-48A6-84FD-152BF63FB14C}" Then
      
        ' проверяем есть ли паддон в другом активном заказе
        ...
        If Not rs Is Nothing Then
         If rs.EOF Then
            ...
            ' проверяем есть ли уже в нашем заказе
            Set Obj = Nothing
...
'загрузка начальных данных при запуске формы
...
'получение значения веса с электронных весов
'Parameters:
' параметров нет
'Returns:
...
Attribute GetWeight4.VB_HelpID = 735
  On Error Resume Next
    Dim ws As String
    Dim ch As String
    Dim start As Single
