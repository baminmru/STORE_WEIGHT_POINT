...
Begin VB.UserControl vpnITTD_COUNTRY_admi 
   ...
   Begin VB.Frame pnlITTD_COUNTRY 
      ...
      Top             =   0
      Width           =   9000
      Begin VB.CommandButton cmdITTD_COUNTRYRun 
         Height          =   330
         Left            =   2700
         Style           =   1  'Graphical
         TabIndex        =   9
         Tag             =   "copy.ico"
         ToolTipText     =   "Коприровать"
         Top             =   30
         Width           =   330
      End
...
Private Sub cmdITTD_COUNTRYRun_Click()
  ...
  MakeRefresh = False
  On Error Resume Next
  Set logic = CreateObject(item.TypeName & "_LOGIC.ITTD_COUNTRY_admi")
  If Not logic Is Nothing Then
    Call logic.OnRun(item, u, Me, UseDefault, MakeRefresh)
    Set logic = Nothing
  End If
  If UseDefault Then
