...
Attribute VB_HelpID = 220
' Окно визарда приемки

Option Explicit
Public StepNo As Integer
Attribute StepNo.VB_VarHelpID = 275
Dim XMLQryCode As String
Dim XMLTheClient As String
Public Item As ITTIN.Application
Attribute Item.VB_VarHelpID = 255
...
Attribute NoMSG.VB_VarHelpID = 260
Public InPoddon As String
Attribute InPoddon.VB_VarHelpID = 245
Public InWeight As String
Attribute InWeight.VB_VarHelpID = 250
Public SinglePoddon As Boolean
Attribute SinglePoddon.VB_VarHelpID = 270
...
' "{E3728A5B-6B62-48BF-9E5A-D4F0BCBFC75B}" 'Приемка завершена
...
Private Sub cmd6FindCell_Click()
  ...
  Dim prule As ITTD_RULE
  On Error Resume Next
  Set prule = Item.ITTIN_DEF.Item(1).ThePartyRule
  
  
  f.country = ""
  If Not prule Is Nothing Then
    If prule.TheCountry = Boolean_Da Then
  ...
  If Not prule Is Nothing Then
    If prule.TheFactory = Boolean_Da Then
  ...
  f.killplace = ""
  If Not prule Is Nothing Then
    If prule.killplace = Boolean_Da Then
...
Private Sub cmdQryCode_Click()
  ...
  If (Not res Is Nothing) Then
    ...
    End If
  End If
  log.message "Приемка " & txtQryCode
End Sub
...
'Найти заказ у в нашей базе
  ...
  If id = "" Then
    ...
    Set rs = conn.Execute("select B.code b_code,e.code e_code,d.code d_code from receiving_line A  join item B on A.item_id =B.id  join receiving_order C on a.order_id = C.id join partner D on c.partner_id= d.id join partner E on b.CLASS= e.CODE where e.code <>d.code and a.order_id='" & qID & "'")
    While Not rs.EOF
...
' открытие строки заказа из грида при двойном клике
...
'  call me.Before1()
Public Sub Before1()
Attribute Before1.VB_HelpID = 225
On Error Resume Next
    txtQryCode.Text = ""
...
'  call me.Before2()
Public Sub Before2()
Attribute Before2.VB_HelpID = 230
  On Error Resume Next
...
On Error Resume Next
  ...
  End If
End Function
'после шага 4 -Параметры груза по умолчанию
...
On Error Resume Next
  If ITTIN_QLINE1.IsOK Then
    ...
    ITTIN_QLINE1.save ' Это перенос данных из формы в объект
    
    
    ' Поверка значения срока годности
    If curQRow.exp_date <= Date + 60 Then
        Dim ms As String
        If curQRow.exp_date = 0 Then
            ms = "Не задан срок годности товара!" & vbCrLf & "Продолжить?"
        Else
            ms = "Срок годности товара истекает : " & Format(curQRow.exp_date, "dd/mm/yyyy") & vbCrLf & _
            "Указанная дата верна?"
        End If
        
        If MsgBox(ms, vbQuestion + vbYesNo, "Контроль срока годности") = vbNo Then
            OK = False
            Exit Function
        End If
    End If
...
'после шага 6- Вес поддона с грузом
...
' предупрежедние  о выходе за границы 10 % коридора по весу короба
  Dim corob As Double
...
'после шага 7 -  Уточнение данных по поддону
Private Function After7() As Boolean
On Error Resume Next
  After7 = False
  If ITTIN_PALET1.IsOK Then
    ITTIN_PALET1.save   ' Это перенос данных из формы в объект
    After7 = True
    On Error GoTo bye
...
'до шага 4 -  Параметры груза по умолчанию
Private Sub Before4()
On Error Resume Next
...
'до шага 6 - Вес поддона с грузом
Private Sub Before6()
On Error Resume Next
...
' "{588C5203-1E59-408E-92A1-B3DFED8C19FA}" 'Списана
  ...
  Dim P As ITTPL_DEF
  log.message "Взвешивание паддона: " & txt4PNum & " вес паддона " & MyRound(txt3Weight)
  
  ' поддон помечается как загруженный
...
'  если надо добавляем в список
  If Not eplOK Then
    With Item.ITTIN_EPL.Add
      Set .TheNumber = P
      .PalWeight = P.Weight
      .save ' добавляем в список пустых паллет
    End With
  End If
...
'до шага 7 -  Уточнение данных по поддону
Private Sub before7()
On Error Resume Next

  Dim ors As ADODB.Recordset
  Dim con As ADODB.Connection
  Set con = Manager.GetCustomObjects("refref")
  Set ors = con.Execute("select NUMBER,ord_date from receiving_order where ID=" & Manager.GetIDFromXMLField(txtQryCode.Tag))
  
  log.message "Паддон: " & txt4PNum & " вес " & MyRound(txt4FullWeight)
  
  curQRow.ITTIN_PALET.Refresh
  
'  создается запись о принятой паллете в памяти
...
'    заполняются поля
    ...
    poddon.QryInDate = ors!ord_date
    poddon.QryInNumber = ors!Number
    ...
    ' тут будет выведено предупреждение об ошибке, если случится беда
'    сохраняем даные по поддону
    poddon.save
    
'    сохраняем изменения даных товара в строке заказа
    curQRow.save
    
'    сохраняем данные по новой паллете
    .save  ' тут необходимо сохранить чтобы нормально отнумеровать строку
    log.message "сохранение в ВК"
...
Private Sub Before9()
  ...
  For i = 1 To Item.ITTIN_QLINE.Count
      pcnt = pcnt + Item.ITTIN_QLINE.Item(i).ITTIN_PALET.Count
  Next
  ...
  Dim srv As ITTD.ITTD_SRV
    ...
    For i = 1 To Item.ITTIN_SRV.Count
      ...
      Set srv = Item.ITTIN_SRV.Item(i).srv
      ...
      If srv.AutoSetPallet = Boolean_Da Then
         If Item.ITTIN_SRV.Item(i).Quantity = 0 Then
                   srvGrid.TextMatrix(i, 1) = pcnt
         End If
      End If
    Next
    
    log.message "заказ закрыт " & txtQryCode
End Sub

'до шага 8 - Документ на поддон с грузом
Private Sub Before8()
 On Error Resume Next
  ...
  While Not OK
    ...
    OK = SaveRCVRowToCore(Item, curQRow, LinePal, txt4NewPlace.Tag, txtQryCode.Text)
    If Not OK Then
       OK = Not MagicMessageBox("Ошибка при сохранении данных в CORE. Паддон: " & pal.brief & vbCrLf & "Будет проведена повторная попытка записи данных.")
       Dim conn As Object
       Set conn = GetCoreConn(True)
  ...
  While OK
    ...
    err.Clear
    curQRow.save
    LinePal.save  ' тут необходимо сохранить чтобы нормально отнумеровать строку
    If err.Number <> 0 Then
        MsgBox err.Description
        OK = True
    End If
...
'обработка смены состояния визарда
'Parameters:
' параметров нет
'See Also:
'  Before1
...
'  StepNo
'Example:
'  call me.ProcessStatus()
Public Sub ProcessStatus()
Attribute ProcessStatus.VB_HelpID = 265
On Error Resume Next
 ' скрываем все фреймы
  ...
  Case 4
  'Шаг4 - Параметры груза по умолчанию
    Before4
    Frame4.Visible = True
    AdjFrame Frame4
  ...
  Case 7
  'Шаг7 - Уточнение данных по поддону
    before7
    Frame7.Visible = True
    AdjFrame Frame7
    SetBtnPos cmdCancel, 3
    SetBtnPos cmdNext, 4
  Case 8
  'Шаг 8 - Документ на поддон с грузом
    Before8
    ...
    ' тут немного меняются позиции кнопок
...
On Error Resume Next
  ...
  Case 6
    ...
    If txt4FullWeight = "" Or txt4FullWeight = "0" Or Not IsNumeric(txt4FullWeight) Then
      ...
      result = False
    ElseIf MyRound(txt4FullWeight) > 1200 Then
      MsgBox "Вес поддона превышает 1200 кг."
      txt4FullWeight = 0
      result = False
    End If
    ...
    If chk4Caliber.Value = vbChecked Then
      If MyRound(txt4CaliberWeight) <= 0 Then
        MsgBox "Значения веса калиброванного товара должны быть больше 0"
        result = False
      End If
    End If
    ...
    If result Then
      ...
      Else
        result = False
        log.message "Отказ от регистрации паллеты: " & txt3Poddon
      End If
    End If
...
Attribute GetWeight4.VB_HelpID = 235
  On Error Resume Next
    Dim ws As String
    Dim ch As String
    Dim start As Single
