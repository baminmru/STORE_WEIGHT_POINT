...
--  перевод описани €чеек в форму нужную оптимизатору
...
as
-- запрос дл€ проверки идентичности товара в €чейке
...
-- дта последней приемки груза на данной паллете
...
-- комплексный запрос по движению товара
-- приемка груза
...
-- состо€ние склада с дешифрованными ссылочными пол€ми
select 
stock.id stock_id
...
as
-- информаци€ по уже отгруженному товару (последний цикл отгрузки)
...
as
-- расчет глобальной выморозки в разрезе одного артикула (item_id)
-- считаем последний оборт паллет
...
-- считаем суммарные значени€ по всем предыдущим оборотам
-- приемка без последнего оборота паллет
select 
...
-- отгрузка без последнего оборота паллет
...
as
-- выморозка отгруппированаа€ по поклажедателю
select 
...
-- список товаров, которые не подлежат автоматической оптимизации
select 
stock.id stock_id
, pallet.code pallet_code
, a.CODE LOC_CODE
...
as
-- зарос показывающий нехватку или превышение по товарам в разрезе кода товара (item_id)
select 
...
-- запрос дл€ апроксимции выморозки на будущие дни
select 
...
as
-- выморозка в разрезе паллеты за последний оборот товара
select a.rectype, b.LastRCV ,a.the_date , a.pallet,a.item_id
 ,in_quantity , in_boxes
 ,out_quantity, out_boxes
 ,out_quantity * datediff(d, b.LastRCV,a.the_date)/30  
...
-- заблокированные по выморозке паллеты
select 
 partner.code partner_code ,item_id, item.code item_code, item.description,
