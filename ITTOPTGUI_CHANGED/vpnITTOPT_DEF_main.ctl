...
'    u.FormattedDocNum = pfx.orgcode & "-ОПТ-" & Right("0000000" & u.DocNum, 7) & "/" & Right("00" & (Year(Date) Mod 100), 2)
'   Else
'    u.FormattedDocNum = "ИТП" & "-ОПТ-" & Right("0000000" & u.DocNum, 7) & "/" & Right("00" & (Year(Date) Mod 100), 2)
'   End If
'   u.Save
 End If
...
Private Sub cmdITTOPT_DEFSave_Click()
  ...
  On Error GoTo bye
    ...
    If editITTOPT_DEF.item.FormattedDocNum = "" Then
      ...
      Set pfx = man.GetCustomObjects("PFX")
      If Not pfx Is Nothing Then
       editITTOPT_DEF.item.FormattedDocNum = pfx.orgcode & "-ОПТ-" & Right("0000000" & editITTOPT_DEF.item.DocNum, 7) & "/" & Right("00" & (Year(Date) Mod 100), 2)
      Else
       editITTOPT_DEF.item.FormattedDocNum = "ИТП" & "-ОПТ-" & Right("0000000" & editITTOPT_DEF.item.DocNum, 7) & "/" & Right("00" & (Year(Date) Mod 100), 2)
      End If
      editITTOPT_DEF.InitPanel
    End If
