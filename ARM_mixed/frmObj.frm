...
Option Explicit
'Форма отображения документа
' Это MDI  контейнер для контролов документов
' Контролы загружаются динамически при открытии формы

Public c As Object
Attribute c.VB_VarHelpID = 605
Public Item As Object
Attribute Item.VB_VarHelpID = 630
Public Host As gui
Attribute Host.VB_VarHelpID = 620
Private mymode As String
Dim mm() As Object


'текущий контрол документа
...
'инициализация формы
'Parameters:
'[IN][OUT]  ObjItem , тип параметра: Object - документ,
'[IN][OUT]   Optional mode , тип параметра: String = ""  - режим
'See Also:
'  c
'  CheckIsNew
'  CheckIsOk
...
Attribute INIT.VB_HelpID = 625
  Set Item = ObjItem
  mymode = mode
again:
  On Error Resume Next
  
'  проверка наличия лицензии на документ в списке лицензий
  err.Clear
...
' загрузка контрола документа на форму
...
'  инициализация контрола документа
...
'  проверка возможности смены состояния
  If RoleDocCanSwitchStatus(Item) Then
  
'    отработка логики при смене состояния
    If BeforeChangeStatus(Item, mm(cmbState.ItemData(cmbState.ListIndex)).Tag) Then
      ...
      'загрузка списка следующих состояний
      CheckStatus
      Me.Hide
      Unload Me
    End If
...
'  variable = me.TestLock()
' Set variable = me.TestLock()
Public Function TestLock()
Attribute TestLock.VB_HelpID = 640
...
'проверка необходимости удаления пустого документа
...
'проверка возможности смены состояния и исполнение доп. логики
...
'  TestLock
'Example:
'  call me.PreLoadState()
Public Sub PreLoadState()
Attribute PreLoadState.VB_HelpID = 635
  On Error Resume Next
...
'Parameters:
'[IN][OUT]  o , тип параметра: Object  - документ
'Returns:
...
Attribute CheckIsOk.VB_HelpID = 615
    'get plain part
    Dim res As Boolean
    Dim p_col As Object
    Dim P As Object
    ...
    While Not rs.EOF
        ...
        If rs!PartType = 0 Then
          ...
          While Not rsf.EOF
              FieldName = rsf!fname & ""
              On Error Resume Next
              Set MyRound = CallByName(P, FieldName, VbGet)
              If err.Number <> 0 Then
                  ...
                  End If
              Else
                  If Not (MyRound Is Nothing) Then
                      CheckIsOk = True
                  End If
              End If
              rsf.MoveNext
...
Attribute CheckIsNew.VB_HelpID = 610
    'get plain part
    Dim res As Boolean
    Dim p_col As Object
    Dim P As Object
